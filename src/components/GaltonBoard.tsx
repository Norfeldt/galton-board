        ...
        <div className="absolute bottom-0 left-0 right-0 flex justify-center items-end h-32">
          <div className="flex w-full mb-4">
            {binCounts.map((count, index) => (
              <div 
                key={index} 
                className="flex-1 text-center text-sm font-semibold text-white bg-gradient-to-t from-slate-900/80 to-slate-700/80 backdrop-blur-sm p-2 first:rounded-bl-lg last:rounded-br-lg border-r border-white/20 last:border-r-0"
              >
                {count}
              </div>
            ))}
